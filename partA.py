...
def main(target_img_dir,patch_pix,mode=2):
    ...
    pool = None
    if mode == 2: # all
        fname = './CS303B_Dataset_for_Assignment_1/manmade_training'
        pool1, dir_pool1 = get_pool_and_dir(fname)
        
        fname = './CS303B_Dataset_for_Assignment_1/natural_training'
    ...
    cv2.imwrite('./composite.png',img_composited)
...
def preprocess_source():
    resize_source('./CS303B_Dataset_for_Assignment_1/manmade_training','./resize_manmade_training')
    resize_source('./CS303B_Dataset_for_Assignment_1/natural_training','./resize_natural_training')
...
def get_pool(dir_pool):
    ...
    if not os.path.isfile(pool_file_name):
        ...
        for i in tqdm.trange(length,desc='making pool'):
            ...
            img = cv2.cvtColor(cv2.imread('resize_'+folder+'/'+filename),cv2.COLOR_BGR2RGB)
